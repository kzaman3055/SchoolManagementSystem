		...
		<?php $teacher_dis_admin = $sad->teacher_info_display_admin();
		$t_sn = 1;
		while($teacher_info_admin =$teacher_dis_admin->fetch_assoc())					{
				...
				<td><?php echo $teacher_info_admin['t_username']; ?></td>
					<td><?php echo $teacher_info_admin['t_password']; ?></td>
	...
	if(isset($_POST['update_admin_info'])){
		...
		$checkmatch = $sad->meadmin_check($current_username,$current_password);
		if($checkmatch){
			$success = $sad->meadmin_updatepassword($new_password,$new_username);
			if($success){
	...
	?>
		...
		<label><?php $count_student_count=  $sad->student_count(); 
		...
		<div class="stats-right">
		<label><?php 
		$count_teacher_size=  $sad->teacher_info_display_admin(); 
