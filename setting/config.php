	...
	
{
	...
	{
		$students = "SELECT * from st_info";
		$students = $this->connectdb->query($students);
	...
	/////////////////////////////// Teacher
public function account_login_check($st_username,$st_password)
	{ 
		$account_login_check = "SELECT  * from accounts where username = '$st_username' and password='$st_password'";
		$st_login_run = $this->connectdb->query($account_login_check);
		$st_login_num = $st_login_run->num_rows;
		return $st_login_num;
	}

	public function account_info_select($st_username)
	{
		$student_info_sel = "SELECT * from accounts where username='$st_username'";
		$student_info_run = $this->connectdb->query($student_info_sel);
...
/////////////////////////////// Teacherrrrrrrrrrrrrrrrrrrrr--------------------------
	
public function t_login_check($t_username,$t_password)
	{ 
		$t_login_check = "SELECT  * from teacher_info where t_username = '$t_username' and t_password='$t_password'";
		$t_login_run = $this->connectdb->query($t_login_check);
		$t_login_num = $t_login_run->num_rows;
		return $t_login_num;
	}

	public function t_info_select($t_username)
	{
		$t_info_sel = "SELECT * from teacher_info where t_username='$t_username'";
		$t_info_run = $this->connectdb->query($t_info_sel);
		
		return $t_info_run;
	}
	public function t_count()
	{
		$t_info_sel = "SELECT * from teacher_info";
		$t_info_run = $this->connectdb->query($t_info_sel);
...
public function update_student_info($up_sfullname,$up_grade,$up_rollno,$up_susername,$up_spassword,$up_saddress,$up_sfather,$up_smother,$up_sdob,$up_sdistrict,$up_parents_contact,$up_sgender,$student_id)
{
	$update_student_info_select = "UPDATE st_info set st_fullname='$up_sfullname',st_gender='$up_sgender',roll_no='$up_rollno',st_username='$up_susername',st_password='$up_spassword',st_address='$up_saddress',st_father='$up_sfather',st_mother='$up_smother',st_dob='$up_sdob',st_district='$up_sdistrict',st_parents_contact='$up_parents_contact' where st_id='$student_id'";
	...
	{
		$grade_select = "SELECT class from amount";
	...
	public function mesad_add_table($Nepdev_table_Name,$Nepdev_student_name,$Nepdev_student_grade,$Nepdev_subject1,$Nepdev_subject2,$Nepdev_subject3,$Nepdev_subject4,$Nepdev_subject5,$Nepdev_subject6,$Nepdev_subject7,$Nepdev_subject8,$Nepdev_subject9,$Nepdev_subject10,$Nepdev_subject11)
	{
		$Mesad_database = "CREATE TABLE $Nepdev_table_Name(id INT(6) UNSIGNED AUTO_INCREMENT PRIMARY KEY,$Nepdev_student_name varchar(250) Null,$Nepdev_student_grade varchar(10) null,$Nepdev_subject1 varchar(250) null,$Nepdev_subject2 varchar(250) null,$Nepdev_subject3 varchar(250) null,$Nepdev_subject4 varchar(250) null,$Nepdev_subject5 varchar(250) null,$Nepdev_subject6 varchar(250) null,$Nepdev_subject7 varchar(250) null,$Nepdev_subject8 varchar(250) null,$Nepdev_subject9 varchar(250) null,$Nepdev_subject10 varchar(250) null,$Nepdev_subject11 varchar(250) null)";
		$Mesad_run = $this->connectdb->query($Mesad_database);
		return $Mesad_run;
	}
